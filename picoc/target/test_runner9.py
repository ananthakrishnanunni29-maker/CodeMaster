...
try:
    # First manually call StdioModule.Setup
    from cstdlib.stdio import StdioModule
    print("Calling StdioModule.Setup...", flush=True)
    StdioModule.Setup(pc)
    print("StdioModule.Setup OK", flush=True)
    
    # Then test LibraryAdd
    from cstdlib.stdio import StdioFunctions
    print(f"Testing LibraryAdd with {len(StdioFunctions)} functions", flush=True)
    CLibraryModule.LibraryAdd(pc, StdioFunctions)
    print("LibraryAdd OK", flush=True)
